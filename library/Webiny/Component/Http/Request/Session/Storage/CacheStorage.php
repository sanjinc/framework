...
namespace Webiny\Component\Http\Request\Session\Storage;
...
use Webiny\Component\Http\Request\Session\SessionStorageInterface;
use Webiny\WebinyTrait;

/**
 * Session storage based on Cache component.
 *
 * @package		 Webiny\Component\Http\Request\Session\Storage
 */
...
class CacheStorage implements SessionStorageInterface{
	 ...
	 *
	 * @throws \Webiny\Component\Http\Request\Session\SessionException
	 */
	function __construct(ConfigObject $config){

		$this->_prefix = isset($config->prefix) ? $config->prefix : 'wfs';
		$this->_ttl = isset($config->ttl) ? $config->ttl : 86400;
		$this->_cacheId = isset($config->storage->params->cache_id) ? $config->storage->params->cache_id : 'wfc';

		try{
			$this->_cacheDriver = $this->cache($this->_cacheId);
		}catch (CacheException $e){
			throw new SessionException('Unable to get cache driver "'.$this->_cacheId.'" for session storage.');
		}
	}
