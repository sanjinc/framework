...
- a loader class that implements `Webiny\Bridge\Image\ImageLoaderInterface`
- an image manipulation class that extends `Webiny\Bridge\Image\ImageAbstract`

After that just change the `bridges.image` library in your config and the framework will use your bridge instead the
default one.

## Configuration

The `Image` component requires very little configuration. Here is an example configuration:
